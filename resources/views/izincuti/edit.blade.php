...
@section('header')
    ...
    <div class="appHeader bg-primary text-light">
        ...
        <div class="pageTittle">Edit Pengajuan Cuti</div>
        <div class="right"></div>
    </div>
@endsection
@section('content')
    <div class="row" style="margin-top: 70px">
        <div class="col">
            <form method="POST" action="/izincuti/{{ $dataizin->kode_izin }}/update" id="formIzin">
                @csrf
                    ...
                    <select name="kode_cuti" id="kode_cuti" class="form-control selectmaterialize">
                        ...
                        @foreach ($mastercuti as $d)
                            <option {{ $dataizin->kode_cuti == $d->kode_cuti ? 'selected' : '' }}
                                value="{{ $d->kode_cuti }}">{{ $d->nama_cuti }}</option>
                        @endforeach
                    </select>
                </div>
